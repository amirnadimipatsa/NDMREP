...
class ComponentTesterApp:
    ...
    def upload_pdf(self):
        name = self.name_var.get().strip()
        if not name:
            messagebox.showwarning("No name", "Enter a component name first.")
            return
        ...
        for i, pdf_path in enumerate(paths):
            dest = os.path.join(PDF_FOLDER, f"{name}_{i+1}.pdf")
            with open(pdf_path, 'rb') as src, open(dest, 'wb') as dst:
                dst.write(src.read())
            saved_files.append(dest)
        messagebox.showinfo("Uploaded", f"{len(saved_files)} file(s) uploaded.")
    ...
    def view_pdf_files(self):
        ...
        if not name:
            messagebox.showwarning("No name", "Enter a component name first.")
            return
        matched = [f for f in os.listdir(PDF_FOLDER) if f.startswith(name)]
        if not matched:
            messagebox.showinfo("No Files", "No datasheets found for this component.")
            return
        for file in matched:
            path = os.path.abspath(os.path.join(PDF_FOLDER, file))
            webbrowser.open_new(path)

    def change_password(self):
        global DEFAULT_PASSWORD
        old = simpledialog.askstring("Old Password", "Enter current password:", show="*")
